...
import { getJsonFromDotEnv, getDotEnvFromJson, showError } from './helpers';
...
export class FileConverter {
	...
	public async convertFiles(files: vscode.Uri[]): Promise<void> {
		const shouldKeepOriginalFiles = false;//TODO: await this.shouldKeepOriginalFiles(files.length);
	...
	private static getNewFileContent(convertFromType: ConvertFromType, oldContent: string) {
		const converter = {
			[ConvertFromType.Json]: getDotEnvFromJson,
			[ConvertFromType.DotEnv]: getJsonFromDotEnv
		}[convertFromType];
	...
	private static getNewFileExtension(convertFromType: ConvertFromType) {
		...
		const toDotEnvFileExtension = getConfig<'.env'>(ConfigId.FileExtensionsDotEnv);

		const fileExtension = {
			[ConvertFromType.Json]: toDotEnvFileExtension,
			[ConvertFromType.DotEnv]: toJsonFileExtension
		}[convertFromType];
